...
export const generateWebsite=async (req,res)=>{
    try {
        ...
        if(!prompt){
            return res.json.status(400).json({message:'prompt is required'});
        }
        const user=await User.findById(req.user._id)
        
        if(!user){
            return res.json.status(400).json({message:'user not found'});
        }

        if(user.credits<50){
            return res.json.status(400).json({message:'you have not enough credits to generate a website.'})
        }
